...
Having a 1D Kalman Filter did not give satisfactory results, so applying a 2D Kalman Filter now, 
State vector is now 2D — both the hedge ratio and the intercept evolve as independent random walks:
State:        [beta_t, alpha_t] = [beta_{t-1}, alpha_{t-1}] + w_t     w_t ~ N(0, Q)
Observation:   price_A_t = beta_t * price_B_t + alpha_t + noise        noise ~ N(0, R)
Q is now a 2×2 process-noise covariance matrix (independent process variances for beta and alpha — I'll keep it diagonal, no cross-correlation assumed), and the observation matrix H_t = [price_B_t, 1] picks out both state components (price_B_t weights beta's contribution, 1 weights alpha's).

The predict/update recursion is the same idea as before, just in matrix form:
Predict:  x_pred = x_{t-1}                    P_pred = P_{t-1} + Q
Update:   H = [price_B_t, 1]
          y_pred = H @ x_pred
          innovation = price_A_t - y_pred
          S = H @ P_pred @ H + R
          K = P_pred @ H / S
          x_t = x_pred + K * innovation
          P_t = P_pred - outer(K, H) @ P_pred
